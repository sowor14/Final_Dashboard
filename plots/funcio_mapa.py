...
def vagueries_plot(df_v: list, show_figure=False):
    ...
    # vagueries=gpd.read_file('/home/usuario/Documentos/Master/1_AVDM/dashbord_migration/vagueries.geojson')
    # vagueries.drop(columns=['id','nom_aft'], inplace=True)
    vagueries.rename(columns={"aft":"codi_vagueria",'nom_aft':'Vegueria'}, inplace=True)
    ...
    mapa=vg_map.plot_bokeh(simplify_shapes=0, category="Migrants", colormap="Colorblind", legend = "Migrants arribats per Vagueria", hovertool_columns=['Vegueria']+["Migrants"],show_figure=False)
